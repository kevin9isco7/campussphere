...
class Config:
    ...
    USE_DATABASE_URL = bool(DATABASE_URL_CONFIG) and not env_bool("DATABASE_FORCE_INDIVIDUAL_VARS", default=False)
    DATABASE_HOST = DATABASE_URL_CONFIG.get("host") if USE_DATABASE_URL else env_value("DATABASE_HOST", "DB_HOST", default="127.0.0.1")
    DATABASE_PORT = DATABASE_URL_CONFIG.get("port") if USE_DATABASE_URL else env_int("DATABASE_PORT", "DB_PORT", default=3306)
    DATABASE_NAME = DATABASE_URL_CONFIG.get("name") if USE_DATABASE_URL else env_value("DATABASE_NAME", "DB_NAME", default="school_management")
    DATABASE_USER = DATABASE_URL_CONFIG.get("user") if USE_DATABASE_URL else env_value("DATABASE_USER", "DB_USER", default="root")
    DATABASE_PASSWORD = DATABASE_URL_CONFIG.get("password") if USE_DATABASE_URL else env_value("DATABASE_PASSWORD", "DB_PASSWORD", default="", allow_empty=True)
    DATABASE_SSL_MODE = DATABASE_URL_CONFIG.get("ssl_mode") if USE_DATABASE_URL else env_value("DATABASE_SSL_MODE", default="", allow_empty=True)
